...
function setupEventListeners() {
    ...
    bodyFocusOutListener = (e) => {
        ...
        if (e.target.matches('.task-name')) {
            if (task.name !== e.target.innerText) updateTask(taskId, { name: e.target.innerText });
...
function render() {
    ...
    if (sortableSections) sortableSections.destroy();
    sortableSections = new Sortable(taskListBody, { handle: '.section-header-list .drag-handle', animation: 150 });
    
    sortableTasks.forEach(st => st.destroy());
    sortableTasks.length = 0;
    document.querySelectorAll('.tasks-container').forEach(container => {
        sortableTasks.push(new Sortable(container, { group: 'tasks', handle: '.fixed-column .drag-handle', animation: 150 }));
    });
}
...
function renderFooter() {
    ...
    project.customColumns.forEach(col => {
        ...
        if (col.aggregation === 'Sum') {
            let total = project.sections.reduce((sum, section) => sum + section.tasks.reduce((secSum, task) => secSum + Number(task.customFields[col.id] || 0), 0), 0);
...
function createTaskRow(task) {
    ...
    const displayName = task.name.trim() ? task.name : `[Add task name]`;
    ...
    rowWrapper.innerHTML = `
        <div class="fixed-column">
            ...
            <span class="task-name ${!task.name.trim() ? 'is-placeholder' : ''}" contenteditable="true">${displayName}</span>
        </div>
        <div class="scrollable-columns-wrapper">
            <div class="scrollable-columns">

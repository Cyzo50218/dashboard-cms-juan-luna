...
//Restore checked state from activeFilters
...
function render() {
    ...
    // --- MODIFICATION START ---
    // Apply filters and sorting to get the data that should be rendered.
    ...
    // Disable drag-and-drop if a programmatic sort is active.
    ...
    // Add visual indicators to buttons if they are active.
...
function renderHeader(projectToRender) {
    ...
    // MODIFIED: Uses the passed-in projectToRender object
...
function renderFooter(projectToRender) {
    ...
    projectToRender.customColumns.forEach(col => {
        ...
        if (col.aggregation === 'Sum') {
            // MODIFIED: Calculates total from the potentially filtered/sorted data
...
function createTaskRow(task) {
    ...
    // Display name is the actual task name. An empty string is fine.
    // The placeholder is handled entirely by CSS for a better UX.
    ...
    // MODIFICATION:
    // The 'task-name' span no longer needs the 'is-placeholder' class.
    // It now uses a 'data-placeholder' attribute. If the 'displayName' is empty,
    // the CSS will automatically show the placeholder text. This works perfectly
    // with the logic that removes empty new tasks when you click away.
...
/**
 * Adds a new task to a section, renders the UI, and focuses on the new task's name field.
 * @param {object} section - The section object to add the task to.
 * @param {string} [position='end'] - Where to add the task ('start' or 'end').
 */
function addNewTask(section, position = 'end') {
    // A temporary 'isNew' flag identifies the task for the focus-out logic.
    ...
    render(); // Re-render the UI to include the new task row.
    
    // After rendering, find the new task element and focus its name field.

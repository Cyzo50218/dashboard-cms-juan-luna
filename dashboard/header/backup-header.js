...
// Assume these module-level variables are available in this scope (from header.js)
// let tasksRecentHistoryData = [];
// let projectsRecentHistoryData = [];
// let recentTasksUnsubscribe = null;
// let recentProjectsUnsubscribe = null;
// let currentUserId = null; // Set by onAuthStateChanged


// Also assume renderRecentItems, fetchRecentTasksFromFirestore,
// and loadProjectRecentHistory (as defined in the previous header.js response) are accessible.
// And other DOM elements like optionBtns, mytaskdisplay, projectdisplay, etc.

optionBtns[0].addEventListener("click", async () => {
    const btn = optionBtns[0];
    const isSelected = btn.classList.contains("selected");

    // Immediately clear content or show a loader if content is dynamic
    recentContainer.innerHTML = '<div class="section-loader"><p>Loading tasks...</p><div class="spinner"></div></div>';

    // Unsubscribe from projects history if it's active
    if (recentProjectsUnsubscribe) {
        recentProjectsUnsubscribe();
        recentProjectsUnsubscribe = null;
    }
    ...
    if (isSelected) {
        ...
        selectedOptionBtnIndex = -1;
        btn.classList.remove("selected");
        optionBtns.forEach(b => b.classList.remove("hide")); // Show all option buttons

        // Re-enable recent tasks listener (if it was stopped) or ensure it's running
        recentTasksUnsubscribe = fetchRecentTasksFromFirestore(
            renderRecentItems,
            exampleRecentPeople, // Pass example people data
            projectsRecentHistoryData, // Pass the global projects history
            exampleRecentMessages, // Pass example messages data
            4, false, false, false
        );

        // Hide specific displays and show general saved search text/container
        mytaskdisplay.classList.add("hidden");
        projectdisplay.classList.add("hidden"); // Ensure project display is hidden
        messagesEmptyState.classList.add("hidden");
        peopleEmptyState.classList.add("hidden");
        savedSearchText.classList.remove("hidden");
        savedSearchContainer.classList.remove("hidden");
        savedContainer.classList.remove("hidden"); // Assuming this is general saved items
        recentContainer.classList.remove("hidden"); // Ensure recent container is visible
        emailContainerId.classList.add('hidden');
        emailContainerPeopleId.classList.add('hidden');
        ...
        // Hide other specific displays
        projectdisplay.classList.add("hidden"); // Ensure project display is hidden
        messagesEmptyState.classList.add("hidden");
        peopleEmptyState.classList.add("hidden");

        // Set up recent tasks listener (if not already running)
        recentTasksUnsubscribe = fetchRecentTasksFromFirestore(
            renderRecentItems,
        ...
        // Adjust visibility of general search elements
        mytaskdisplay.classList.remove("hidden");
        savedSearchText.classList.add("hidden");
        savedSearchContainer.classList.add("hidden");
        recentContainer.classList.remove("hidden"); // Ensure recent container is visible
    ...
    // Common actions after selection/deselection
    input.value = ''; // Clear input on option selection
    cancelIcon.classList.add('hidden'); // Hide cancel icon
    searchOptions.classList.remove("hidden"); // Always show filter buttons
});
...
optionBtns[1].addEventListener("click", async () => {
    ...
    // Immediately clear content or show a loader
    recentContainer.innerHTML = '<div class="section-loader"><p>Loading projects...</p><div class="spinner"></div></div>';

    // Unsubscribe from tasks history if it's active
    if (recentTasksUnsubscribe) {
        recentTasksUnsubscribe();
        recentTasksUnsubscribe = null;
    }
    ...
    if (isSelected) {
        ...
        // Re-enable recent tasks listener (default view after deselecting Projects)
        recentTasksUnsubscribe = fetchRecentTasksFromFirestore(
            renderRecentItems,
            exampleRecentPeople, // Pass example people data
            projectsRecentHistoryData, // Pass the global projects history
        ...
        // Hide specific displays and show general search text/container
        projectdisplay.classList.add("hidden");
        mytaskdisplay.classList.add("hidden"); // Ensure mytask display is hidden
        messagesEmptyState.classList.add("hidden");
        peopleEmptyState.classList.add("hidden");
        savedSearchText.classList.remove("hidden");
        savedSearchContainer.classList.remove("hidden");
        ...
        // --- Is NOT selected, now selecting "Projects" ---
        selectedOptionBtnIndex = 1;
        btn.classList.add("selected");
        ...
        // Set up recent projects listener
        ...
        // Adjust visibility of general search elements
        projectdisplay.classList.remove("hidden"); // Show projects display
        savedSearchText.classList.add("hidden");
        savedSearchContainer.classList.add("hidden");
        recentContainer.classList.remove("hidden"); // Ensure recent container is visible
        emailContainerId.classList.add('hidden');
...
// The remaining click handlers for optionBtns[2] (People) and optionBtns[3] (Messages)
// will need similar adjustments to ensure they correctly:
// 1. Unsubscribe from other active listeners (tasks, projects).
// 2. Call the appropriate `fetch...FromFirestore` or `load...RecentHistory` function.
// 3. Pass the *global* data arrays (tasksRecentHistoryData, projectsRecentHistoryData, etc.) to renderRecentItems.
// 4. Manage the visibility of your display elements (`mytaskdisplay`, `projectdisplay`, `peopleQueryDiv`, `messagesEmptyState`, etc.).

optionBtns[2].addEventListener("click", async () => { // People Button
    const btn = optionBtns[2];
    const isSelected = btn.classList.contains("selected");

    // Clear previous dynamic content and show loader
    recentContainer.innerHTML = '<div class="section-loader"><p>Loading people...</p><div class="spinner"></div></div>';

    // Unsubscribe from other real-time listeners
    if (recentTasksUnsubscribe) { recentTasksUnsubscribe(); recentTasksUnsubscribe = null; }
    if (recentProjectsUnsubscribe) { recentProjectsUnsubscribe(); recentProjectsUnsubscribe = null; }
    ...
    if (isSelected) {
        ...
        // Revert to default view (e.g., recent tasks)
        fetchRecentTasksFromFirestore(renderRecentItems, exampleRecentPeople, projectsRecentHistoryData, exampleRecentMessages, 4, false, false, false);
        ...
        recentContainer.classList.remove("hidden");
        messagesEmptyState.classList.add("hidden");
        peopleEmptyState.classList.add("hidden"); // Hide people empty state if default view
        mytaskdisplay.classList.add("hidden");
        projectdisplay.classList.add("hidden");
        emailContainerId.classList.add('hidden');
        emailContainerPeopleId.classList.add('hidden'); // Hide people invite
    } else {
        selectedOptionBtnIndex = 2;
        btn.classList.add("selected");
        ...
        // Only render people. No new listener needed here as exampleRecentPeople is static.
        renderRecentItems([], exampleRecentPeople, [], [], 4, false, true, false); // Pass only people data
        ...
        // Specific to People: Show people-related elements
        if (peopleQueryDiv) peopleQueryDiv.classList.remove('hidden'); // Show people query div
        emailContainerPeopleId.classList.remove('hidden'); // Show people invite button
        if (recentContainerTitle) recentContainerTitle.classList.add("hidden"); // Hide general recent title if people is specific
        recentContainer.classList.add("hidden"); // Hide recent container as people are rendered separately
    }
    input.value = '';
    cancelIcon.classList.add('hidden');
    searchOptions.classList.remove("hidden");
...
optionBtns[3].addEventListener("click", async () => { // Messages Button
    const btn = optionBtns[3];
    const isSelected = btn.classList.contains("selected");

    // Clear previous dynamic content and show loader
    recentContainer.innerHTML = '<div class="section-loader"><p>Loading messages...</p><div class="spinner"></div></div>';
    ...
    if (isSelected) {
        ...
        savedSearchContainer.classList.remove("hidden");
        recentContainer.classList.remove("hidden");
        messagesEmptyState.classList.add("hidden"); // Hide messages empty state if default view
        peopleEmptyState.classList.add("hidden");
        mytaskdisplay.classList.add("hidden");
        projectdisplay.classList.add("hidden");
        emailContainerId.classList.add('hidden');
        ...
        // Only render messages. No new listener needed here as exampleRecentMessages is static.
        renderRecentItems([], [], [], exampleRecentMessages, 4, false, false, true); // Pass only messages data
        ...
        // Specific to Messages: Show messages empty state or container if applicable
        messagesEmptyState.classList.remove("hidden");
        recentContainer.classList.remove("hidden"); // Ensure messages are rendered in recentContainer
    }
    input.value = '';
    cancelIcon.classList.add('hidden');
    searchOptions.classList.remove("hidden");
